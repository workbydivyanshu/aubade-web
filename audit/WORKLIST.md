...
Card typography was added as a seventh pass after the counts showed the reference
...
the reference, not left-aligned, and back/forward were never missing from our top bar.

**Still open — the search field.** The reference's `/search` presents a collapsed
...
Two captures of the reference and Aubade playing the same album at 2880x1800 allow
...
| --- | --- |
| Hero card radius 0px vs 16px | The reference "0px" boxes matched are the 1440×900 page containers, not a hero. The reference's home has no hero card; ours is our own element with no counterpart. |
...
The reference is a streaming service; these have no local-library meaning. Revisit as features, not polish.
...
Three agents found this independently and it is confirmed: the reference's dominant radius is
...
| Element | Reference | Aubade | Delta |
| --- | --- | --- | --- |
...
The reference sets explicit px line-heights; we use unitless multipliers, so every text block
...
Also: artist section-header tracking is −0.6px in the reference, −0.176px in ours.
...
The reference uses **176×176 (75 instances) and 224×224 (40)**; we only use 176. Worth checking
